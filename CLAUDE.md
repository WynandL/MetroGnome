...
- `audio/selftest/` — Acoustic-loopback mic self-test ("Groove Check"): `MicSelfTest.kt` plays clicks/claps through the speaker, listens back, and grades the device (GOOD_FIT / USABLE / NOT_FIT). Design principle: the scoring sweep is the sole arbiter of fitness; upstream measurements (latency jitter, split drift, click leak rate) either calibrate the device or annotate the report - they never disqualify. The only hard fails are things calibration cannot fix: no output timestamp, implausible latency, undetectable claps, true two-axis click/clap inseparability. On PASS, `SelfTestCalibrationStore` persists the latency constant plus per-device `ClapDetector` thresholds for both acceptance axes (`clap_band_ratio`, `clap_flatness_min`); `MicCalibration` is the single read surface all three mic features use. `SelfTestThresholds.GATE_LOGIC_VERSION` stamps stored verdicts - bump it on any gate change that could flip outcomes (a stale FAIL then reads as never-tested and the check is re-offered; a PASS is never invalidated). Outcomes report anonymously to Firestore `mic_self_test` via `cloud/MicCheckReporter` including the spectral margins.
