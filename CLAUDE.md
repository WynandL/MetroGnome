...
- `MetroFcmService` — `FirebaseMessagingService` subclass; builds and posts the notification (guarded by a live `POST_NOTIFICATIONS` check, since a granted-at-send-time assumption can be stale).
...
- `NotificationTopics` — broadcast model: every consenting device subscribes to one topic, `"all_users"`. No per-device tokens are collected or stored — a message sent to that topic from the Firebase Console (or a Firestore-triggered Cloud Function via the Admin SDK) reaches everyone with zero backend.
